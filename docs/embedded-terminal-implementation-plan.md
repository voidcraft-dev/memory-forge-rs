...
本功能的目标是在 Memory Forge 内部运行交互式 CLI，并通过独立“会话”工作区统一管理：

- 一键在应用内部恢复或分支当前会话。
- 自动使用会话原始工作目录和已有 Resume/Fork 命令。
- 支持全屏 TUI、ANSI 颜色、光标、键盘、中文输入法、滚动和选择。
- 从历史详情启动后自动进入独立“会话”页面。
- 切换到总览、设置或其他平台时，正在运行的终端继续工作；返回后恢复滚动历史。
...
- 页面路由切换不得停止 PTY；前端为每个终端保留有界输出历史，返回“会话”页时重建 xterm 视图。
- 用户关闭终端会话时，如果子进程仍运行，需要确认。
- 子进程自然退出后保留最后输出和退出码，用户可“重新启动”或关闭会话。
...
左侧主导航新增可显示、隐藏和排序的“会话”入口。该页面使用“集合 + 工作区”布局：
...
- 集合中显示来源平台、会话标题、Resume/Fork、cwd、状态和打开时间。
- 历史详情只保留启动入口，避免长文本与终端争夺主区域。
- 菜单排序单独持久化为 `navigationItems`，不把“会话”当作 AI 平台参与统计。
...
interface EmbeddedTerminalPanelProps {
  ...
  onOpenExternal: () => void;
  onClose: () => void;
  children?: React.ReactNode; // 后续接入真实 xterm viewport
}
```
...
1. 本文档作为共同规格。
2. 在 `feature/embedded-terminal` 完成纯 UI 提交。
3. 审查 UI 改动并修复集成问题。
4. 实现 xterm、Rust PTY、IPC、生命周期和性能控制。
5. 完成自动测试、人工视觉验收和 Windows 打包。
6. 用户确认后再决定版本号、合并和发布。
